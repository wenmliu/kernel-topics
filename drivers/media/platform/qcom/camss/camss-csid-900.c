...
/*
 * RDI configuration. The full and lite CSID share the same register layout;
 * only the RDI block base differs.
 */
#define CSID_RDI_CFG0(rdi) \
	((csid_is_lite(csid) ? 0x600 : 0x1300) + 0x200 * (rdi))
...
#define CSID_RDI_CTRL(rdi) \
	((csid_is_lite(csid) ? 0x604 : 0x1304) + 0x200 * (rdi))
#define		RDI_CTRL_START_CMD			BIT(0)

#define CSID_RDI_CFG1(rdi) \
	((csid_is_lite(csid) ? 0x610 : 0x1310) + 0x200 * (rdi))
...
/* RDI Pixel Store Configuration (full CSID only) */
#define CSID_RDI_PIX_STORE_CFG0(rdi)		(0x1314 + 0x200 * (rdi))
...
static void __csid_configure_rdi_stream(struct csid_device *csid, u8 enable, u8 port, u8 vc)
{
	...
	/* Configure pixel store using dedicated register in full CSID */
