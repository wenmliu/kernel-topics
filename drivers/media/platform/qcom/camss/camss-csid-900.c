...
/* Register Update Commands, RUP/AUP (v900 splits RUP/AUP with a SET) */
...
/*
 * v900's RDI path registers share the gen3/980 field layout but sit at a
 * new base: 0x1300 (full) / 0x600 (lite), with a 0x200 stride.
 */
...
/* RDI IRQ status in wrapper (0x114 + 0x10 * rdi, full and lite alike) */
...
static void __csid_rup_aup_update(struct csid_device *csid, int port_id)
{
	/*
	 * v900 splits the RUP and AUP commands and requires an explicit SET
	 * write to CSID_RUP_AUP_CMD to make the changes take effect.
	 */
	csid->reg_update |= CSID_RUP_AUP_RDI(port_id);
	writel(csid->reg_update, csid->base + CSID_RUP_CMD);
	writel(csid->reg_update, csid->base + CSID_AUP_CMD);
...
static void csid_subdev_reg_update(struct csid_device *csid, int port_id, bool clear)
{
	if (clear)
		...
		__csid_rup_aup_update(csid, port_id);
}
